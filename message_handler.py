...
def should_forward(
    ...
    触发关键词: list,
    触发艾特机器人: bool,
    approve_deny_启用: bool,
    approve_deny_允许用户: list
) -> Tuple[bool, str]:
    """
    判断是否需要转发消息给 Hermes。
    ...
    if approve_deny_启用 and 消息内容.startswith(("approve", "deny")):
        if next((seg.text for seg in event.get_messages() if isinstance(seg, Plain)), '').strip().startswith("/"):
            if not _check_approve_deny_permission(用户id, approve_deny_允许用户):
