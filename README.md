...
| `/重启爱马仕` | 执行重启 Hermes 命令（别名：`/重启Hermes`） |
...
| **其他配置** | `启用Hermes连接报告` | WebSocket 连接成功时自动发送状态报告 |
| | `发送目标类型` / `发送目标ID` | 连接报告发送到群聊或私聊的目标 |
| | `重启Hermes命令` | `/重启爱马仕` 指令执行的 shell 命令（默认 `systemctl restart hermes`） |
...
| v6.6 | **指令逻辑与完善**：黑白名单逻辑优化、跟随框架唤醒、自动获取bot实例                      |
| v7.0 | **重启 Hermes + 连接报告**：群聊一键重启 Hermes、WebSocket 连接状态自动报告、自定义重启命令 |
| v7.2 | **多实例 self_id 过滤**：多 OneBot 适配器场景下按机器人qq过滤事件、类名修正 |
| v7.2.2 | **重启指令加固**：管理员权限校验 + 未启用连接报告/未配置重启命令时拒绝执行 |
| v7.2.3 | **OneBot 并发发现**：多候选并发探测、按 self_id 精确绑定、平台加载后自动发现 |
| v7.2.4 | **非阻塞发现**：create_task 避免阻塞加载；on_astrbot_loaded 二次绑定 |
