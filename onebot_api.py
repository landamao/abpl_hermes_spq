...
async def send_text(session: aiohttp.ClientSession, onebot_url: str, 群号: int, 消息内容: str, token: str = "") -> dict:
    ...
    try:
        logger.debug(f"[Hermes适配器] OneBot 发送文本请求: {payload}")
        async with session.post(url, json=payload, headers=headers) as resp:
            result = await resp.json()
            logger.debug(f"[Hermes适配器] OneBot 发送文本结果: {result}")
            return result
...
async def send_cq(session: aiohttp.ClientSession, onebot_url: str, 群号: int, 消息内容: list, token: str = "") -> dict:
    ...
    if token:
        headers["Authorization"] = f"Bearer {token}"
    try:
        logger.debug(f"[Hermes适配器] OneBot 发送CQ请求: {payload}")
        async with session.post(url, json=payload, headers=headers) as resp:
            result = await resp.json()
            logger.debug(f"[Hermes适配器] OneBot 发送CQ结果: {result}")
            return result
    except Exception as e:
        logger.error(f"[Hermes适配器] OneBot CQ 发送失败: {e}", exc_info=True)
        return {"error": str(e)}
...
async def send_private(session: aiohttp.ClientSession, onebot_url: str, 用户id, 消息内容: str, token: str = "") -> dict:
    ...
    try:
        ...
        logger.error(f"[Hermes适配器] 发送私聊消息失败: {e}", exc_info=True)
        return {"error": str(e)}
...
async def upload_group_file(session: aiohttp.ClientSession, onebot_url: str, 群号: int, 文件路径: str, 文件名: str = "", token: str = "") -> dict:
    ...
    try:
        async with session.post(url, json=payload, headers=headers) as resp:
            result = await resp.json()
            logger.info(f"[Hermes适配器] OneBot 群文件上传结果: {result}")
            return result
    except Exception as e:
        logger.error(f"[Hermes适配器] 群文件上传失败: {e}", exc_info=True)
        return {"error": str(e)}
...
async def upload_private_file(session: aiohttp.ClientSession, onebot_url: str, 用户id, 文件路径: str, 文件名: str = "", token: str = "") -> dict:
    ...
    try:
        async with session.post(url, json=payload, headers=headers) as resp:
            result = await resp.json()
            logger.info(f"[Hermes适配器] OneBot 私聊文件上传结果: {result}")
            return result
    except Exception as e:
        logger.error(f"[Hermes适配器] 私聊文件上传失败: {e}", exc_info=True)
        return {"error": str(e)}
...
async def set_msg_emoji_like(session: aiohttp.ClientSession, onebot_url: str, message_id: str|int, emoji_id: str|int = 12, token: str = "") -> dict:
    ...
    if token:
        headers["Authorization"] = f"Bearer {token}"
    try:
        logger.debug(f"[Hermes适配器] OneBot 表情回应请求: payload={payload}")
        async with session.post(url, json=payload, headers=headers) as resp:
            result = await resp.json()
            logger.debug(f"[Hermes适配器] OneBot 表情回应结果: {result}")
            return result
    except Exception as e:
        logger.error(f"[Hermes适配器] 表情回应失败: {e}", exc_info=True)
        return {"error": str(e)}
...
async def handle_api_request(
    ...
    logger.info(f"[Hermes适配器] 收到 API 请求: {动作}, echo={数据.get('echo', '')}")
    ...
    try:
        if 动作 == 'send_group_msg':
            ...
            logger.warning(f"[Hermes适配器] 未支持的 API 操作: {动作}")
            logger.debug(f"原始数据：{数据}")
            return {"status": "failed", "retcode": 100, "msg": f"未支持的操作: {动作}", "data": None}

    except Exception as e:
        logger.error(f"[Hermes适配器] API 调用失败: {动作} - {e}", exc_info=True)
