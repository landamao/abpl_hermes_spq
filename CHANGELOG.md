...
### 🐛 修复
- **修复自定义重启命令未执行**：`run_command` 内部的 `yield` 语句使函数变为生成器，导致 `subprocess.run` 在子线程中从未被执行。移除 `run_command` 内的 `yield`，改用 `logger.error` 记录错误
- **修复自动艾特时 CQ 码丢失解析**：当消息为 str 类型（可能包含 CQ 码）时，原来会转成结构化 list 导致 CQ 码变成纯文本。改为直接拼接 `[CQ:at,qq=xxx]` 前缀，保留原始 CQ 码的解析
