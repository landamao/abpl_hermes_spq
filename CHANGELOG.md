...
### 🔧 更新
- **OneBot 实例发现重写**：`_discover_bot_instance` 支持多候选并发探测，并按配置 `self_id` 精确匹配
  - 收集所有 `CQHttp` 候选实例，并发调用 `get_login_info`（最多 6 次、间隔 5 秒）
  - 配置了机器人 QQ 时，仅绑定匹配 `self_id` 的实例；未配置则取第一个成功实例
  - 发现成功后直接 `set_event`，日志输出实际加载的 `self_id`
- **平台加载后自动发现**：新增 `@filter.on_platform_loaded()` 钩子，平台就绪后自动重新发现 OneBot 实例
- **初始化路径简化**：`消息发送方式=框架已有的WebSocket` 时直接调用 `_discover_bot_instance()`，不再在外层二次包装
...
### 🔧 更新
- **重启指令权限加固**：`/重启爱马仕`（`/重启Hermes`）增加管理员权限校验
- **重启前置检查**：未启用 Hermes 连接报告或未配置重启命令时，直接提示并拒绝执行
